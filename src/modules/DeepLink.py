...
class DeepLinkAnalyzer:
    def __init__(self):
        ...
        self.addURI_pattern = re.compile(
            r'\.addURI\(([^,]+),\s*([^"]+),\s*(\d+)\)', re.IGNORECASE
        )
    ...
    def searching_activity(self, activities):
        ...
        for activity in activities:
            activity_name_pattern = re.compile(
                r'<activity([^>]+)android:name="([^"]*)"', re.IGNORECASE
            )
                        ...
                        for parameter in deeplink_params:
                            ...
                            if "R.string" in parameter:
                                ...
                                if match:
                                    ...
                                    package = file_path.split("java_src\\")[1].split(
                                        "\\resources"
                                    )[0]
                                    xml_file = os.path.join(
                                        os.getcwd(),
                                    ...
                                    real_parameter = self.resolve_string_key(
                                        xml_file, string_key
                                    )
                            ...
                            # 변수인 경우 값을 해석
                                ...
                                real_parameter = self.resolve_variables(
                                    whole_path, parameter
                                )
