...
class SQLInjectionAnalyzer:
    def __init__(self):
        ...
        self.external_package_pattern = re.compile(r"package\s+(com\.google\.|androidx\.|com\.android\.)|import\s+(com\.instabug\.library\.model\.session\.SessionParameter|androidx\.room\.)", re.IGNORECASE)
    ...
    def has_prevention_pattern(self, content):
        return (self.input_validation_pattern.search(content) or 
                self.orm_pattern.search(content) or 
                self.prepared_statement_pattern.search(content) or 
                self.additional_pattern.search(content))
