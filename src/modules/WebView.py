...
class WebViewAnalyzer:
    ...
    def __init__(self):
        ...
        self.setallowFileaccess_pattern = re.compile(r"setAllowFileAccess(true)", re.IGNORECASE)
        ...
        self.imported_file_pattern = re.compile(r"(?<=import\s)com.[\w.]+(?=;)", re.IGNORECASE)
    ...
    def permissions_in_app_webview(self,xml_path, keyword):
        ...
        dangerous_keywords, dangerous_permissions = analyzer.run_webview(xml_path, keyword)
        ...
        if file_path.endswith("AndroidManifest.xml"):
            ...
            for activity in exported_activities:
                ...
                if os.path.exists(whole_path):
                    ...
                    fileaccess_lines = self.extract_lines_with_fileaccess_patterns(ExtractContent(whole_path).extract_content())
                    if webview_lines:
                        ...
                        for imported_file in imported_files:
                            whole_path = os.path.join(base_path, "java_src", package, "sources", imported_file.replace(".", os.sep) + ".java")
                            js_result.extend(self.javascript_analysis(whole_path))
                            js_result = list(set(js_result))
                            if js_result:
                                dangerous_keywords, dangerous_permission = self.permissions_in_app_webview(file_path,js_result)
                                #print(f"DeepLinkAnalyzer: {dangerous_keywords}, {dangerous_permission}")
                                #self.output.append((dangerous_keywords, dangerous_permission))
                        ...
                        result_dict = {
                            ...
                            "javascript_lines": list(set(js_result)),  # 중복 제거
                        }
